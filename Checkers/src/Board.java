	...
	private boolean validateWhiteMovements(int index, int newIndex) 
	{
		...
		if (piece.equals(WHITEPIECE) && newPiece.equals(BLANK)) {
			...
			if (index % 8 == 0) {
				...
				else if ((newIndex == index - 14 && board.get(index - 7).equals(BLACKPIECE))
						|| (newIndex == index + 18 && board.get(index + 9).equals(BLACKPIECE))) {
					isValid = true;					
				}
			}
		...
		// mesma cor.
			...
			else if (Math.abs(newIndex - index) % 9 == 0 && !(index % 8 == 0)) {
				isValid = true;				
				int count = 0;
	...
	{		
		...
		if (piece.equals(BLACKPIECE) && newPiece.equals(BLANK)) {
			...
			if (index % 8 == 0) {
				...
				// pecas pretas que comem 
						...
						|| (newIndex == index - 14 && (board.get(index - 7).equals(WHITEPIECE) || board.get(index - 7).equals(WHITEDAMA))))
					isValid = true;
			}
			...
			// as pecas da direita apenas se podem mover para a esqureda
				...
				// pecas pretas que comem 
						...
						|| (newIndex == index - 18 && (board.get(index - 9).equals(WHITEPIECE) || board.get(index - 9).equals(WHITEDAMA))))
					isValid = true;
			}
			// pecas no meio da board
			else {
				...
				// pretas que comem V
				else if ((newIndex == index + 14 && (index%8 - newIndex%8) > 0 && (board.get(index + 7).equals(WHITEPIECE) || board.get(index + 7).equals(WHITEDAMA)))
						|| (newIndex == index + 18 && (newIndex%8 - index%8) > 0 &&(board.get(index + 9).equals(WHITEPIECE) || board.get(index + 9).equals(WHITEDAMA)))
						|| (newIndex == index - 14 && (index%8 - newIndex%8) > 0 && (board.get(index - 7).equals(WHITEPIECE) || board.get(index - 7).equals(WHITEDAMA)))
						|| (newIndex == index - 18 && (newIndex%8 - index%8) > 0 && (board.get(index - 9).equals(WHITEPIECE) || board.get(index - 9).equals(WHITEDAMA))))
					isValid = true;
			}
		}
		...
		// mesma cor.
			...
			if ((Math.abs(newIndex - index) % 7 == 0)) {
				...
				int count = 0;				
				for (int i = index + 9; i < newIndex; i += 9) {
					if (board.get(i).equals(WHITEPIECE) || board.get(i).equals(BLACKDAMA))
						count++;
					if (count >= 1)
						isValid = false;
				}
